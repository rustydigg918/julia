...
loop: // while (i <= lx) {
    ...
    if (!env->vy && env->j < env->ly && jl_is_vararg_marker(jl_tparam(env->yd, env->j)))
        env->vy += 1;
...
static int obvious_subtype(jl_value_t *x, jl_value_t *y, jl_value_t *y0, int *subtype)
{
    ...
    if (jl_is_vararg_marker(x)) {
        if (!jl_is_vararg_marker(y)) {
            *subtype = 0;
            return 1;
        }
        return 0;
...
static jl_value_t *intersect_tuple(jl_datatype_t *xd, jl_datatype_t *yd, jl_stenv_t *e, int param)
{
    ...
    int vx=0, vy=0, vvx = (lx > 0 && jl_is_vararg_marker(jl_tparam(xd, lx-1)));
    int vvy = (ly > 0 && jl_is_vararg_marker(jl_tparam(yd, ly-1)));
    if (!vvx && !vvy && lx != ly)
        return jl_bottom_type;
    ...
    while (1) {
        ...
        if (xi && jl_is_vararg_marker(xi)) vx = 1;
        if (yi && jl_is_vararg_marker(yi)) vy = 1;
        if (xi == NULL || yi == NULL) {
            res = jl_bottom_type;
